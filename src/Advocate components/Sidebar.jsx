        ...
        <ul className="space-y-1 px-2">
          {menuItems.map(item => (
            <li key={item.id}>
              ...
              >
                <span className={`${currentPath === item.id.toLowerCase() ? 'text-white' : 'text-gray-400'}`}>
                  {item.icon}
                </span>
                {!collapsed && <span className="ml-3">{item.name}</span>}
                {currentPath === item.id.toLowerCase() && !collapsed && (
        ...
        {!collapsed ? (
          <>
            ...
            <button 
              ...
              className="w-full flex items-center justify-center py-2 px-4 bg-gray-800 hover:bg-gray-700 rounded-lg text-sm transition-colors text-gray-300"
            >
